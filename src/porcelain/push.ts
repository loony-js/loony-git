...
export async function push(
  ...
  for (const { localRef, remoteRef } of refspecs) {
    ...
    if (localSha === remoteSha) {
      lines.push(
        `= [up to date]  ${localRef.replace("refs/heads/", "")} -> ${remoteRef.replace("refs/heads/", "")}`,
      );
      continue;
    }
    ...
    if (remoteSha !== "0".repeat(40) && !opts.force) {
      ...
      if (!isFF) {
        lines.push(
          `! [rejected]  ${localRef} -> ${remoteRef} (non-fast-forward)`,
        );
        continue;
      }
    }
    ...
    lines.push(
      ` ${arrow}  ${localRef.replace("refs/heads/", "")} -> ${remoteRef.replace("refs/heads/", "")}`,
    );
  }
...
function resolveRefspecs(
  ...
  if (refspec) {
    ...
    const remoteRef = (remote ?? local).startsWith("refs/")
      ? (remote ?? local)
      : `refs/heads/${remote ?? local}`;
